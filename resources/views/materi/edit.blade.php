...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold mb-6">Edit Materi Pembelajaran</h1>
    ...
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <form action="{{ route('materi.update', $materi->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="judul" id="judul" value="{{ $materi->judul }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>

            <div class="mb-4">
                <label for="deskripsi" class="block text-gray-700 text-sm font-bold mb-2">Deskripsi</label>
                <textarea name="deskripsi" id="deskripsi" rows="4" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>{{ $materi->deskripsi }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="tipe" id="tipe" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                    <option value="gambar" {{ $materi->tipe == 'gambar' ? 'selected' : '' }}>Gambar</option>
            ...
            <div class="mb-6">
                ...
                @if($materi->file)
                    <p class="text-sm text-gray-500 mt-1">File saat ini: {{ basename($materi->file) }}</p>
                @endif
            </div>
