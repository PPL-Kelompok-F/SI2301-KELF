...
class SubmissionController extends Controller
{
    ...
    public function store(Request $request, $id)
    {
        ...
        $deadlineStart = \Carbon\Carbon::parse($assignment->deadline_start)->timezone('Asia/Jakarta');
        $deadlineEnd = \Carbon\Carbon::parse($assignment->deadline_end)->timezone('Asia/Jakarta');

        if ($now->lt($deadlineStart)) {
            return back()->with('error', 'Assignment belum dibuka.');
        }

        $submitStatus = $now->gt($deadlineEnd) ? 'late' : 'ontime';
